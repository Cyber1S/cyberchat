...
Cyber1SChat comes with predefined set of colors for buttons. The color of the button can be set with `color-scheme` to `woot-button`.
...
There are diffrent styles that comes with cyber1schat buttons. This can set with `variants` prop for `woot-button`. By deault the filled will be the style, other variations are `smooth, hollow, clear, link`
