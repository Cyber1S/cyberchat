<template>
  <div class="resolve-actions">
    ...
    >
      <wooh-dropdown-menu>
        <wooh-dropdown-item v-if="!isPending">
          <wooh-button
            variant="clear"
        ...
        <wooh-dropdown-divider v-if="isOpen" />
        <wooh-dropdown-sub-menu
          v-if="isOpen"
...
import WoohDropdownSubMenu from 'shared/components/ui/dropdown/DropdownSubMenu.vue';
...
export default {
  ...
  data() {
    return {
      ...
      STATUS_TYPE: woohConstants.STATUS_TYPE,
    };
  },
  computed: {
    ...mapGetters({ currentChat: 'getSelectedChat' }),
    isOpen() {
      return this.currentChat.status === woohConstants.STATUS_TYPE.OPEN;
    },
    isPending() {
      return this.currentChat.status === woohConstants.STATUS_TYPE.PENDING;
    },
    isResolved() {
      return this.currentChat.status === woohConstants.STATUS_TYPE.RESOLVED;
    },
    isSnoozed() {
      return this.currentChat.status === woohConstants.STATUS_TYPE.SNOOZED;
    },
    buttonClass() {
  ...
  methods: {
    async handleKeyEvents(e) {
      ...
      if (hasPressedAltAndEKey(e)) {
        ...
        try {
          await this.toggleStatus(woohConstants.STATUS_TYPE.RESOLVED);
        } catch (error) {
          // error
        }
